...
const imagesDir = "C:\\Users\\hp\\.gemini\\antigravity\\brain\\c4b55416-a585-4436-809d-b6bb67e2d694";
...
async function run() {
  ...
  const usersToCreate = [
    ...
    { email: "vendor-approved@test.com", role: "vendor", status: "approved", shop_name: "Fresh Foods Supermarket" },
    { email: "vendor-pending@test.com", role: "vendor", status: "pending", shop_name: "Pending Store" },
    { email: "vendor-rejected@test.com", role: "vendor", status: "rejected", shop_name: "Rejected Store" },
    { email: "vendor-suspended@test.com", role: "vendor", status: "suspended", shop_name: "Suspended Store" },
  ];

  const uids = {};

  for (const u of usersToCreate) {
    try { await auth.getUserByEmail(u.email).then(r => auth.deleteUser(r.uid)); } catch(e) {}
    const rec = await auth.createUser({ email: u.email, password: "Password@123", emailVerified: true });
  ...
  const productTypes = ["apple", "banana", "mango", "tomato", "potato", "onion", "milk", "cheese", "butter", "bread"];
  const prodUrls = {};
  ...
  for (const status of ["pending", "approved", "rejected", "suspended"]) {
    ...
    await db.collection("vendorApplications").doc(appId).set({
      user_id: uid,
    ...
    await db.collection("applicationDocuments").doc(`doc-${appId}-pan`).set({ application_id: appId, doc_type: "pan", file_url: panUrl, isTestData: true });
    await db.collection("applicationDocuments").doc(`doc-${appId}-gst`).set({ application_id: appId, doc_type: "gst", file_url: gstUrl, isTestData: true });
    await db.collection("applicationDocuments").doc(`doc-${appId}-shop_photo`).set({ application_id: appId, doc_type: "shop_photo", file_url: shopPhotoUrl, isTestData: true });
    await db.collection("applicationDocuments").doc(`doc-${appId}-shop_license`).set({ application_id: appId, doc_type: "shop_license", file_url: fssaiUrl, isTestData: true });
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const catMap = { apple: "Fruits", banana: "Fruits", mango: "Fruits", tomato: "Vegetables", potato: "Vegetables", onion: "Vegetables", milk: "Dairy", cheese: "Dairy", butter: "Dairy", bread: "Bakery" };
